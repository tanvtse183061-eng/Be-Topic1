...
export default function DealerQuotation() {
  ...
  const handleCreateQuotation = async (e) => {
    ...
    try {
      // Chuẩn bị params theo API
      const params = {};
      if (formData.evmStaffId) params.evmStaffId = formData.evmStaffId;
      if (formData.discountPercentage) params.discountPercentage = parseFloat(formData.discountPercentage);
      if (formData.notes) params.notes = formData.notes;

      console.log("📤 Params tạo báo giá:", params);

      const createRes = await dealerQuotationAPI.createQuotationFromOrder(formData.dealerOrderId, params);
