...
import { FaShoppingCart, FaUsers, FaCar, FaMoneyBillWave, FaExclamationCircle, FaSpinner, FaClock } from 'react-icons/fa';
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        // Lấy xe trong kho từ VehicleInventory API, chỉ lấy xe có status "available"
        let inventoryData = [];
        try {
          const inventoryRes = await inventoryAPI.getInventory();
          inventoryData = inventoryRes.data || [];
          const availableVehicles = inventoryData.filter(v => {
            const status = (v.status || '').toLowerCase();
            return status === 'available' || status === 'có sẵn';
          });
          setVehicleCount(availableVehicles.length);
        } catch (inventoryErr) {
          console.warn("⚠️ Không thể lấy inventory (có thể không có quyền):", inventoryErr);
