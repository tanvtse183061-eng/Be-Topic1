  ...
  const fetchAll = async () => {
    try {
      ...
      if (vehiclesData.length > 0) {
        ...
        console.log("📦 Sample vehicle licensePlate:", vehiclesData[0].licensePlate);
      }
    ...
    try {
      ...
      await inventoryAPI.deleteInventory(inventoryIdStr);
        ...
        setVehicles(prev => prev.filter(v => {
          const vid = v.id || v.inventoryId || v.vehicleId;
          return String(vid) !== String(inventoryIdStr);
        }));
        return;
      } catch (deleteError) {
        ...
        if (deleteError.response?.status === 404 || deleteError.response?.status === 405) {
          ...
          for (const status of softDeleteStatuses) {
            try {
              ...
              await inventoryAPI.updateStatus(inventoryIdStr, status);
