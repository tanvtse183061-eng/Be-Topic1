  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit && selectedUser) {
        ...
        // ✅ Cập nhật dealerId CHỈ cho STAFF và MANAGER
        if (selectedUser.role === "STAFF" || selectedUser.role === "MANAGER") {
          ...
          if (formData.dealerId !== selectedUser.dealer?.dealerId) {
            updateData.dealerId = String(formData.dealerId).trim();
          }
        } else {
          // EVM_STAFF, ADMIN: Xóa dealerId nếu có
          if (selectedUser.dealer?.dealerId) {
        ...
        // ✅ Backend yêu cầu userType thay vì role
        // Map role từ form sang userType cho backend:
        // STAFF -> DEALER_STAFF
        ...
        if (roleFromForm === "STAFF") {
          userType = "DEALER_STAFF";
        } else if (roleFromForm === "MANAGER") {
          userType = "DEALER_MANAGER";
        } else if (roleFromForm === "EVM_STAFF") {
          userType = "EVM_STAFF";
        } else if (roleFromForm === "ADMIN") {
          userType = "ADMIN";
        } else {
          userType = roleFromForm; // Fallback
        }
        
        console.log("🔍 Role từ form:", roleFromForm);
        console.log("🔍 UserType sẽ gửi:", userType);
        ...
        const userData = {
          ...
          userType: userType, // Backend yêu cầu userType
          role: roleFromForm, // Giữ lại role để tương thích
          isActive: true,
        };

        // ✅ Thêm dealerId CHỈ cho DEALER_STAFF và DEALER_MANAGER
        // EVM_STAFF và ADMIN: KHÔNG gửi dealerId
        if (userType === "DEALER_STAFF" || userType === "DEALER_MANAGER") {
          if (!formData.dealerId || formData.dealerId.trim() === "") {
            throw new Error("Vui lòng chọn đại lý!");
          }
          userData.dealerId = String(formData.dealerId).trim();
          console.log("✅ Thêm dealerId cho", userType, ":", userData.dealerId);
        } else if (userType === "EVM_STAFF") {
          // EVM_STAFF: KHÔNG gửi dealerId hoặc dealerName
          delete userData.dealerId;
          delete userData.dealerName;
          console.log("✅ EVM_STAFF: Không gửi dealerId/dealerName");
        } else if (userType === "ADMIN") {
          // ADMIN: Không gửi dealerId
          delete userData.dealerId;
          delete userData.dealerName;
          console.log("✅ ADMIN: Không gửi dealerId/dealerName");
        }
        ...
        // Xóa các field null/undefined/empty trước khi gửi
        ...
        console.log(
          "📤 Dữ liệu gửi tạo user (sau cleanup):",
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  {isEdit ? (
                    ...
                    <>
                      ...
                      >
                        💡 STAFF và MANAGER cần chọn đại lý. EVM_STAFF và ADMIN không cần.
                      </small>
                    </>
                  )}
                </div>
