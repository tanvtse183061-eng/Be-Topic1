...
public class DealerOrderController {
    ...
    public ResponseEntity<?> deleteDealerOrder(@PathVariable UUID dealerOrderId) {
        try {
            ...
            // Kiểm tra phân quyền: ADMIN, DEALER_STAFF, DEALER_MANAGER có thể xóa order
            if (!securityUtils.hasAnyRole("ADMIN", "DEALER_STAFF", "DEALER_MANAGER")) {
                Map<String, String> error = new HashMap<>();
                error.put("error", "Access denied. Only admin, dealer staff, or dealer manager can delete dealer orders");
                return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
            }
            
            // Kiểm tra dealer user chỉ có thể xóa order của dealer mình
            if (securityUtils.isDealerUser()) {
                ...
                if (currentUser.getDealer() != null) {
                    ...
                    if (!dealerOrder.getDealer().getDealerId().equals(userDealerId)) {
                        Map<String, String> error = new HashMap<>();
                        error.put("error", "Access denied. You can only delete orders for your own dealer");
                        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
                    }
                }
            }
