...
public class DealerQuotationController {
    ...
    public ResponseEntity<?> deleteQuotation(@PathVariable UUID quotationId) {
        try {
            ...
            // Kiểm tra phân quyền: ADMIN, DEALER_STAFF, DEALER_MANAGER có thể xóa quotation
            ...
            // Kiểm tra dealer user chỉ có thể xóa quotation của dealer mình
            if (securityUtils.isDealerUser()) {
                ...
                if (currentUser.getDealer() != null) {
                    ...
                    if (quotation.getDealer() == null || !quotation.getDealer().getDealerId().equals(userDealerId)) {
                        Map<String, String> error = new HashMap<>();
                        error.put("error", "Access denied. You can only delete quotations for your own dealer");
                        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
                    }
                }
            }
